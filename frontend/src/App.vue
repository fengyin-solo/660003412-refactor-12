      ...
      <div class="bg-slate-800 rounded-lg p-4 border border-slate-700">
        ...
        <template v-if="!detailRoot">
          <div class="flex gap-2 mb-3">
            ...
            <button @click="store.clearCognateQuery()" :disabled="store.isCognateEmpty" class="bg-slate-700 disabled:opacity-40 rounded px-3 py-1.5 text-sm">清除</button>
            <button @click="restoreQuery" :disabled="!store.canRestoreCognateQuery" class="bg-cyan-700 disabled:opacity-40 rounded px-3 py-1.5 text-sm">恢复上次查询</button>
          </div>
          <div ref="tableScrollRef" @scroll="onTableScroll" class="overflow-x-auto max-h-64 overflow-y-auto">
            <table class="w-full text-xs">
              ...
              <tbody>
                <tr v-for="cs in store.filteredCognates" :key="cs.root" :data-root="cs.root"
                    @click="openCognate(cs.root)"
                    :class="['border-t border-slate-700 hover:bg-slate-700 cursor-pointer', store.selectedCognateRoot === cs.root ? 'bg-slate-700' : '']">
                ...
                <tr v-if="store.hasNoCognateMatch">
                  <td colspan="8" class="px-2 py-6 text-center text-slate-500">
                    没有匹配的同源词，试试更换关键词或语系。
                    <button v-if="store.canRestoreCognateQuery" @click="restoreQuery" class="ml-2 text-cyan-400 underline">恢复上次查询</button>
                  </td>
                </tr>
              </tbody>
            </table>
        ...
        <div v-else class="bg-slate-900 rounded p-4 space-y-3">
          <div class="flex items-center justify-between">
            <div class="font-mono text-lg font-bold text-cyan-400">{{ detailCognate?.root }}</div>
            <button @click="backFromDetail" class="bg-slate-700 rounded px-3 py-1 text-sm">返回词表</button>
          </div>
          <div class="text-sm text-slate-400">{{ detailCognate?.meaning }} · {{ detailCognate?.period }}</div>
          <div v-if="detailCognate" class="grid grid-cols-2 gap-2 text-xs">
            <div v-for="(word, lang) in detailCognate.languages" :key="lang" class="bg-slate-800 rounded px-2 py-1.5">
              <span class="text-slate-500">{{ lang }}：</span><span class="font-mono text-slate-200">{{ word }}</span>
            </div>
          </div>
        </div>
      </div>
...
import { useEtymologyStore, COGNATE_SETS, LANGUAGE_FAMILIES } from './store/etymology'
...
// 详情面板与词表互斥展示，条件/选择/滚动位置的恢复统一走 store 流程。
const detailRoot = ref<string | null>(null)
const detailCognate = computed<CognateSet | undefined>(() =>
  detailRoot.value ? COGNATE_SETS.find(cs => cs.root === detailRoot.value) : undefined
)

function onTableScroll() {
  if (tableScrollRef.value) store.setCognateScrollTop(tableScrollRef.value.scrollTop)
}

function openCognate(root: string) {
  store.openCognate(root)
  detailRoot.value = root
}

function restoreQuery() {
  store.restoreCognateQuery()
  // 恢复后列表重新渲染，按同一过滤结果定位到上次选择的行与滚动位置。
...
function backFromDetail() {
  ...
  store.closeCognate(root ?? undefined)
  nextTick(scrollToSelected)
}
...
function scrollToSelected() {
  ...
  if (!el) return
  el.scrollTop = store.cognateScrollTop
  const root = store.selectedCognateRoot
  if (root) {
    const row = el.querySelector<HTMLElement>(`tr[data-root="${CSS.escape(root)}"]`)
